...
"""
.. module:: get_idlv_st
   :synopsis: Computing IDLV standarized score - using only dictionaries
   The equation used to compute IDLV value is:
   .. math:: D^{st}_{w,k,m}(V_j) = sum_i=1 to n ( w^((i-1/k-i)^m) * (f_ji / I) )
   where:
    :math:D(V_j) is the lexical availability of word j for a given category
   ## i = position of word j in a given list
   ## f = number of mentions of word j in position i for a given category
   ...
   ## k = position in wich we want the score (or weight in the resulting list) is w.
   ## w = the weigth, between 0 and 1, we want in position k
   ## m = constant what module the intensity to which we want to apply the weigths. (m<=1 first position decay fast; m>1 first positition decay very slow)
...
.. date_last_updated:: Nov 12st, 2019
"""
...
def main_idlv(
    ...
    k : intenger
        Specify the position in where the weight is w
    w : double, optional
        Specify the weigth in position k. (default is 0.1)
    m : double, optional
        Speficy the degree of decay in the weigth of the first elements in resulting list. (default is 1.0)
    ...
    -------
    mylist_tup
        An array of tuples [(key, value), (key, value),...] where key is a word in the vocabulary and value its idlv_standarized score
    """
    stats = {}
    stats[
        "last_position"
    ...
    for word in dict_words_arrayindex:
        ...
        for i in range(last_index + 1):
            ...
            if (
                freq_index != 0
            ):  ## if word j do not apper in index i, there is nothing to add.
                exponent = math.pow((i / (k - 1)), m)
                exponent_w = math.pow(w, exponent)
                sum += exponent_w * (freq_index / max_freq)

                logging.debug(
                    "index=%d; freq_index=%d; max_freq=%d; exponent=%f; exponent_w=%f; sum=%f"
                    % (i, freq_index, max_freq, exponent, exponent_w, sum)
                )
...
def main(dir_in, dir_out, resolution, k, w, m):
    ...
    k : intenger
        Specify the position in where the weight is w
    w : double
        Specify the weigth in position k.
    m : double
        Speficy the degree of decay in the weigth of the first elements in resulting list.
    """
    ...
    logging.info("k:%d, w=%f, m=%f" % (k, w, m))
    ...
    for f in files:
        idlv_file = (
            "IDLV_list_" + get_file_name(f) + "_" + str(resolution) + "r" + "_st.txt"
        )
        idlv_file = os.path.join(dir_out, idlv_file)
        samples = open(f, "r")

        main_idlv(
            idlv_file, samples, resolution, k, w, m
        )  ## remember w, and m are optional in main_idlv

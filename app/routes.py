...
from app import get_idlv as idlvmod
from app import get_idlv_st as idlvmodst
...
UPLOAD_FOLDER = os.path.abspath("app/Archivos/")
...
def subeArchivoPrueba():
	...
	nombre_carpeta = ""
	#Revisa si existen la carpeta de Archivos y de Output para que se pueda guardar el archivo del usuario
	if os.path.exists('app/Archivos'):
		shutil.rmtree('app/Archivos') #Elimina la carpeta Archivos

	os.mkdir('app/Archivos') #Crea una carpeta llamada Archivos
	...
	shutil.copy('app/static/archivos/Mañaneras.zip', contenedor )#Copia el arcivo de prueba (Mañaneras) en la carpeta contenedora (app/Archivos)
...
def Select_files(): #funcion para procesar los archivos señalados
	if request.method == 'POST':
		...
		if os.path.exists('app/Output'):
			shutil.rmtree('app/Output') #Elimina la carpeta OUTPUT

		if os.path.exists('app/static/csv'):
			shutil.rmtree('app/static/csv') #Elimina la carpeta OUTPUT

		os.mkdir('app/Output')   #Crea una carpeta llamada Output
		os.mkdir('app/static/csv')   #Crea una carpeta llamada Output
...
def resultados():
	if os.path.isfile('app/Archivos/Poblaciones.json'):
